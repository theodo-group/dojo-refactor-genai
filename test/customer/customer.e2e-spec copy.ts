...
import { CustomersFixtures } from "test/fixtures/customers-fixtures";

describe.skip("CustomerController (e2e)", () => {
  let app: INestApplication;
  let customerFixtures: CustomersFixtures;
  ...
  beforeAll(async () => {
    ...
    // Initialize customerFixtures
    customerFixtures = new CustomersFixtures(app);
  });

  afterAll(async () => {
    await customerFixtures.clear();
    await app.close();
  });

  describe("/api/customers", () => {
    it("GET / should return all active customers", async () => {
      const customers = await customerFixtures.createCustomers();
