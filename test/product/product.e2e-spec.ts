...
describe('ProductController (e2e)', () => {
  ...
  let productFixtures: ProductFixtureFactory;
  ...
  beforeAll(async () => {
    ...
    productFixtures = new ProductFixtureFactory(app);
    await productFixtures.init(app);

    // Create complete product menu
    productFixtures.addCompleteMenu();
    await productFixtures.build();
  });

  afterAll(async () => {
    await productFixtures.cleanup();
    await app.close();
  });
  ...
  describe('/api/products', () => {
    it('GET / should return all available products', () => {
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(res.body.length).toBe(productFixtures.getData().length);
    ...
    it('GET /:id should return product by id', () => {
      const product = productFixtures.getData()[0];
    ...
    it('DELETE /:id should soft delete a product', () => {
      const product = productFixtures.getData()[1];
