...
The goal is to create a clean testing architecture that is not tightly coupled between modules (no interdependencies between tests).
Each E2E tests must be using their own fixture suite under [./test].
You have to create a fixture factory that can build scenarios accordingly to what each test needs to use.

# Context

Customers can create orders for a list of products. Customers can have discounts on their order total amount thanks to loyalty.

<file name="app-e2e-spec" path="test/app.e2e-spec.ts">
...
<file name="module-e2e-spec" path="test/order/order.e2e-spec.ts">
...
<file name="global-fixtures" path="test/fixtures/global-fixtures.ts">
